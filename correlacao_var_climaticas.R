...
## Dados Notas de florescimento quinzenal (22 quinzenas)
```{r}
...
for (i in 1:4) {
  ...
  tmedD[[i]] <- ggplot(data = subset(dadosTotal,dadosTotal$Epoca.x==i), aes(x=QuinN,y=value)) +
    ...
    scale_y_continuous(name = 'Temperatura m?dia', 
                       sec.axis = sec_axis(~ . /40,name = "M?dia Florescimento"),
                       breaks = seq(0, 30, by = 10),limits = c(0, 30))+
    
    theme(axis.text.y  = element_text(color = 'blue',size = 14),
          axis.title.y = element_text(color='blue',size = 14),
          axis.text.y.right =  element_text(color = 'salmon2',size = 14),
          axis.title.y.right = element_text(color='salmon2',size = 14),
  ...
  tminD[[i]] <- ggplot(data = subset(dadosTotal,dadosTotal$Epoca.x==i), aes(x=QuinN,y=value)) +
    ...
    geom_bar(stat="identity", width=0.5, aes(y=value*30),size=1,color='salmon', 
             fatten = 1,fill="salmon") + theme_bw() +
    scale_y_continuous(name = 'Temperatura m?nima', 
                       sec.axis = sec_axis(~ (. -b)/a,name = "M?dia Florescimento"),
                       breaks = seq(0, 30, by = 10),limits = c(0, 30))+
  ...
  tmaxD[[i]] <- ggplot(data = subset(dadosTotal,dadosTotal$Epoca.x==i), aes(x=QuinN,y=value)) +
    ...
    theme_bw() +
    scale_y_continuous(name = 'Temperatura m?xima', 
                       sec.axis = sec_axis(~ . /40,name = "M?dia Florescimento"),
  ...
  PrecD[[i]] <- ggplot(data = subset(dadosTotal,dadosTotal$Epoca.x==i), aes(x=QuinN,group=1)) +
    ...
    scale_y_continuous(name = 'Precipita??o', 
                       sec.axis = sec_axis(~ . /250,name = "M?dia Florescimento"),
...
tiff(file = "Plot_Var_Clima.tiff", width = 30, height = 45, res = 700, compression = "lzw", units = "cm")
...
### Correlações entre Florescimento e Var climáticas
```{r}
colnames(dadosTotal)
...
res
tiff(file = "Plot_Corr_Clima.tiff", width = 30, height = 45, res = 700, compression = "lzw", units = "cm")
do.call("grid.arrange", c(tmed,Prec,GDA,GDAAc, ncol = 4,nrow=4))
dev.off()
