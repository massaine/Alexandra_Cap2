...
summary(modelconj2)$varcomp
n.env <- 4
vpredict(modelconj2 , h2 ~ V1 / ( V1 + (V3/n.env) + (V5/(2*n.env)) ) )

Var_Cor <- as.data.frame(do.call(rbind,modelconj$sigma)) 
Var_Cor$Componemte <- rep(c("Acession_name:Epoca","Units"),each=6)
...
for (i in 1:length(traits)) {  
   vblup <- mean(diag(modelconj$PevU$`Acession_name:Epoca`[[i]]))
   parametros[i,2] <- rownames(Var_Cor)[i]
   parametros[i,3] <- round(Var_Cor[i,i]/(Var_Cor[i,i] + (Var_Cor[length(traits)+i,i]/3)),2)
   parametros[i,4] <- 1 - (vblup / (2 * Var_Cor[i,i])) ## cullis
   parametros[i,5] <- round(subset(model[[j]]$Beta,model[[j]]$Beta$Effect=="(Intercept)")[i,3],2)
