...
dados <- read.table(here::here("data", "Planilha epocas Maio22_v2.txt"), header=T, sep="\t", na.strings="NA")
...
dados1 <- read.table(here::here("data", "DadosEpocas_NotaFlores.txt"), header=T, sep="\t", na.strings="NA", strip.white=TRUE, check.names=FALSE)
...
dados3 <- dados2[,c("Acession_name","bloco","plant_number","Epoca",
                    "DAPR300", "NumEvRam","Altura.total","CompR",
                    "NM","NP")]

blupsL <- list()
model <- list()
analise <- list()
parametros <- matrix(NA,ncol=7,nrow=5)
parametrosL <- list()

for (j in 1:4) {
  
  model[[j]] <-  mmer(fixed = cbind(DAPR300, NumEvRam,CompR,NM,NP) ~ bloco, tolparinv = 0.5,
            random = ~ Acession_name, getPEV = T, data = subset(dados3,dados3$Epoca==j) %>%                     droplevels(.$bloco))
  ...
  #vblup <- sqrt(mean(diag(model[[1]]$PevU$Acession_name[[i]])))
   ...
   parametros[i,3] <- round(Var_Cor[i,i]/(Var_Cor[i,i] + (Var_Cor[5+i,i]/3)),2)
   ...
   parametros[i,7] <- round(Var_Cor[5+i,i],2)
